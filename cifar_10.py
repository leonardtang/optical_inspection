...
class SimpleCNN(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        x = self.batch1(x)   # Normalize activations into non-linearity
        x = F.relu(x)
        x = F.dropout(x, p=0.1)
        x = self.pool(x)
        ...
        x = x.view(-1, 128 * 4 * 4)
        x = self.fc1(x)
        x = self.batch4(x)   # Normalize activations into non-linearity
        x = F.relu(x)
        x = F.dropout(x, p=0.5)  # Drop out after non-linearity
